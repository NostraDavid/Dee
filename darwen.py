from Dee import Relation, Key, GENERATE
...
class darwen_Database(Database):
    ...
    def _vinit(self):
        ...
        if "C_ER" not in self:
            ...
            self.C_ER = Relation(
                ["CourseId", "Exam_Result"], self.vC_ER, {"pk": (Key, ["CourseId"])}
            )
    ...
    def vC_ER(self):
        return self.COURSE.extend(
            ["Exam_Result"],
            lambda t: {
                "Exam_Result": (self.EXAM_MARK & GENERATE({"CourseId": t.CourseId}))(
                    ["StudentId", "Mark"]
                )
            },
